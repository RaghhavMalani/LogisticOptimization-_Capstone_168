...
This file classifies the datasets currently present in the repository and how they should feed the engineering-ready terminal.
...
| `data/portwatch/port_activity_summary.csv` | Derived from PortWatch | Latest per port | `portid`, `canon`, `name`, `lat`, `lon`, `portcalls`, `import_tonnes`, `export_tonnes`, vessel class counts | Port registry, map pins, national summary | Current best tabular port lat/lon source in root data. |
| `data/portwatch/history/port*.csv` | Derived from PortWatch | Daily per selected port | `date`, `portcalls`, `import`, `export` | Time series fallback for port pages and model inputs | File names are PortWatch numeric IDs; needs registry mapping for readability. |
...
| `india-portwatch/data/geo/india_ports.geojson` | Existing app geo registry | Port point | `name`, `portid`, geometry coordinates | Real map coordinate reference | Useful for map correctness; not all Lovable demo codes match these IDs directly. |
...
| `outputs/forecasts/forecast_table.csv` | Forecast runner | Port forecast horizon | TFT/baseline forecast endpoint and cockpit | Contains quantiles and confidence; model field identifies baseline versus TFT. |
