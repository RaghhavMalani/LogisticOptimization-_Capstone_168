...
import { fetchFleetVessels } from "@/services/fleet";
    ...
    <div className="h-full grid grid-cols-1 grid-rows-[auto_1fr] gap-2">
      <div className="grid grid-cols-6 gap-2">
        <Metric
          ...
          value={fleet.length}
          tone="cyan"
          sub="8 vessels tracked"
        />
        <Metric
          label="AT RISK"
          value={countFleetAtRisk()}
          tone="red"
          sub="wait risk > 0.6"
        />
        <Metric
            ...
            {fleet.map((f) => (
              <button
                ...
                className={cn(
                  ...
                  sel === f.imo
                    ? "bg-[var(--color-cyan)]/10 border-l-2 border-l-[var(--color-cyan)]"
              ...
              >
                ...
                <span className="text-[var(--color-cyan)]">
                  {getPortSnapshot(f.destinationPortCode).short}
                </span>
                ...
                >
                  {f.entryRisk.toFixed(2)}
                </span>
                <span className="text-right tabular-nums text-[var(--color-mint)]">
                  {f.bestArrival}
                </span>
        ...
        <div className="grid grid-rows-[auto_1fr_auto] gap-2 min-h-0">
          <Panel title={`SELECTED · ${v.name}`}>
            <div className="p-3 grid grid-cols-2 gap-2 text-[11px]">
              <div>
                <div className="label-xs">IMO</div>
                <div className="tabular-nums text-[var(--color-cyan)]">
                  {v.imo}
                </div>
              </div>
              <div>
                <div className="label-xs">TYPE</div>
                <div>{v.vesselType}</div>
              </div>
              <div>
                <div className="label-xs">DEST</div>
                <div className="text-[var(--color-cyan)]">{port.name}</div>
              </div>
              <div>
                <div className="label-xs">ETA</div>
                <div className="tabular-nums">{v.eta}</div>
              </div>
              <div>
                <div className="label-xs">WAIT RISK</div>
                <Bar
                  value={v.waitRisk}
                  tone={
                    v.waitRisk > 0.7
              ...
              <div>
                <div className="label-xs">ENTRY RISK</div>
                <Bar
                  value={v.entryRisk}
                  tone={
                    v.entryRisk > 0.6
              ...
              <div>
                <div className="label-xs">BUFFER</div>
                <div className="tabular-nums text-[var(--color-mint)]">
                  {v.buffer}
                </div>
              </div>
            </div>
            ...
            <div className="p-3">
              <div className="grid grid-cols-10 gap-[3px]">
                {Array.from({ length: 10 }).map((_, i) => {
                  const r = [
                    0.42, 0.55, 0.68, 0.79, 0.86, 0.74, 0.62, 0.51, 0.44, 0.38,
                  ][i];
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center gap-1">
                      ...
                      <div
                        ...
                        style={{
                          background: `linear-gradient(180deg, ${bg} 0%, oklch(0.14 0.02 240) 100%)`,
                          opacity: 0.85,
                        }}
                      />
                      <div
              ...
              <div className="flex justify-between text-[9px] text-[var(--color-muted-foreground)] mt-2">
                <span>BEST {v.bestArrival}</span>
                <span>PEAK RISK T+5</span>
                <span>WORST {v.worstArrival}</span>
              </div>
            </div>
          </Panel>
          ...
          <Panel title="ADVISORY · AI SHIP MANAGER">
            <div className="p-3 text-[11px] leading-relaxed border-l-2 border-[var(--color-cyan)]">
              Hold current SOG and adjust ETA to{" "}
              <span className="text-[var(--color-mint)]">{v.bestArrival}</span>{" "}
              to avoid CONGESTED_HIGH regime at {port.name}. Expected berth wait
              drops from{" "}
              <span className="text-[var(--color-red)]">22h → 6h</span> with
              buffer{" "}
              <span className="text-[var(--color-cyan)]">{v.buffer}</span>. Fuel
              penalty ≈ <span className="text-[var(--color-amber)]">+2.4%</span>
              . Confidence{" "}
              <span className="text-[var(--color-cyan)]">
                {v.confidence.toFixed(2)}
              </span>
