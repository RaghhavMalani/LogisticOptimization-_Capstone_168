...
import { FLEET, PORTS } from "@/data/portwatch";
...
function FleetPage() {
  ...
  const v = FLEET.find(f=>f.imo===sel)!;
  const port = PORTS.find(p=>p.short===v.destPort)!;
  ...
  return (
    <div className="h-full grid grid-cols-1 grid-rows-[auto_1fr] gap-2">
      <div className="grid grid-cols-6 gap-2">
        ...
        <Metric label="AT RISK"         value={FLEET.filter(f=>f.waitRisk>0.6).length} tone="red"  sub="wait risk > 0.6" />
          ...
          <div className="text-[10px]">
            ...
            {FLEET.map(f=>(
              <button key={f.imo} onClick={()=>setSel(f.imo)} className={cn(
                ...
                <Chip tone={f.type==="TANKER"?"amber":f.type==="LNG"?"purple":f.type==="CONT"?"cyan":"mint"}>{f.type}</Chip>
                <span className="text-[var(--color-cyan)]">{f.destPort}</span>
                ...
                <span className="text-right tabular-nums text-[var(--color-mint)]">{f.best}</span>
                <span className="text-right tabular-nums text-[var(--color-red)]">{f.worst}</span>
        ...
        <div className="grid grid-rows-[auto_1fr_auto] gap-2 min-h-0">
          <Panel title={`SELECTED · ${v.name}`}>
            <div className="p-3 grid grid-cols-2 gap-2 text-[11px]">
              ...
              <div><div className="label-xs">TYPE</div><div>{v.type}</div></div>
              ...
              <div><div className="label-xs">CONF</div><div className="tabular-nums text-[var(--color-cyan)]">{v.conf.toFixed(2)}</div></div>
          ...
          <Panel title="ARRIVAL WINDOW · 10 DAYS">
            <div className="p-3">
              ...
              <div className="flex justify-between text-[9px] text-[var(--color-muted-foreground)] mt-2"><span>BEST {v.best}</span><span>PEAK RISK T+5</span><span>WORST {v.worst}</span></div>
            </div>
          </Panel>

          <Panel title="ADVISORY · AI SHIP MANAGER">
            <div className="p-3 text-[11px] leading-relaxed border-l-2 border-[var(--color-cyan)]">
              Hold current SOG and adjust ETA to <span className="text-[var(--color-mint)]">{v.best}</span> to avoid CONGESTED_HIGH regime at {port.name}. Expected berth wait drops from <span className="text-[var(--color-red)]">22h → 6h</span> with buffer <span className="text-[var(--color-cyan)]">{v.buffer}</span>. Fuel penalty ≈ <span className="text-[var(--color-amber)]">+2.4%</span>. Confidence <span className="text-[var(--color-cyan)]">{v.conf.toFixed(2)}</span>.
            </div>
          </Panel>
        </div>
      </div>
