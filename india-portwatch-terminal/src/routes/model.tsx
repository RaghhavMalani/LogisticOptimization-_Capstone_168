...
import { listGatingWeights, listModelPipelineStatuses } from "@/services/modelService";
...
function ModelPage() {
  ...
  return (
    <div className="h-full grid grid-rows-[auto_1fr_auto] gap-2">
      <Panel title={`AI PIPELINE · ${port.name.toUpperCase()} · WEATHER → NEWS/NLP → SAR/AIS → DEMAND → HSMM → TFT → DECISION`}>
          ...
          {pipeline.map((s,i)=>(
            <div key={s.key} className="flex-1 panel p-2 relative">
              <div className="flex items-center justify-between">
                ...
                <Chip tone={s.confidence>0.85?"mint":s.confidence>0.7?"cyan":"amber"}>{(s.confidence*100).toFixed(0)}%</Chip>
              </div>
              ...
              <div className="mt-1 text-[9px] text-[var(--color-muted-foreground)]">{s.effectOnForecast}</div>
      ...
      <div className="min-h-0 grid grid-cols-[1.4fr_1fr] gap-2">
        <Panel title="EXPERT CHAIN · INPUT → EFFECT">
          <div className="p-2">
            ...
            {pipeline.map((step)=>(
              <div key={step.key} className="grid grid-cols-[70px_1fr_1fr_60px_50px_80px] gap-2 items-center px-1 py-1.5 border-b border-[var(--color-line)]/40 text-[10px]">
                <Chip tone={step.key==="DEC"?"cyan":step.key==="HSMM"?"amber":step.key==="TFT"?"cyan":"muted"}>{step.key}</Chip>
                <span className="text-[var(--color-foreground)]">{step.inputSignal}</span>
                <span className="text-[var(--color-muted-foreground)]">{step.effectOnForecast}</span>
                <span className="text-right tabular-nums">{step.score.toFixed(2)}</span>
                <span className="text-right tabular-nums text-[var(--color-cyan)]">{step.confidence.toFixed(2)}</span>
                <span className="text-right tabular-nums text-[var(--color-muted-foreground)]">{step.timestamp}</span>
              </div>
            ))}
          </div>
        </Panel>
          ...
          <Panel title="MoE GATING WEIGHTS">
            <div className="p-3 space-y-1.5 text-[11px]">
              {gatingWeights.map(({key, value})=>(
                <div key={key} className="grid grid-cols-[54px_1fr_40px] items-center gap-2">
                  <span className="text-[var(--color-cyan)]">{key}</span>
                  <Bar value={value} tone="cyan"/>
                  <span className="text-right tabular-nums">{(value*100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </Panel>
