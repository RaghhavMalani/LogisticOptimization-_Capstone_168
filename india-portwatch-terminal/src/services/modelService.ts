...
export function getHSMMRegime(portCode: string): HSMMRegime {
  return (
    hsmmRegimes.find((regime) => regime.portCode === portCode) ?? hsmmRegimes[0]
  );
}
...
export function getDecisionRecommendation(
  ...
  return {
    ...
    portCode,
    severity:
      regime.probabilities.severe > 0.75
        ? "severe"
        : regime.probabilities.congested > 0.55
          ? "high"
          : "medium",
    ...
    rationale:
      "Weather, SAR/AIS queue growth, and HSMM dwell-state persistence all point to elevated delay risk.",
