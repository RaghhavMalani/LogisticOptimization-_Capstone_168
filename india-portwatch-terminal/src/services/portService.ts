...
import type { Chokepoint, ChokepointRoute, Port, PortRisk } from "@/types/portwatch";
...
export function getPort(portCodeOrName: string): Port {
  ...
  return (
    ports.find((port) => port.code === normalized || port.short === normalized) ??
    ports.find((port) => port.name.toUpperCase().includes(normalized)) ??
    ports[0]
  );
}

export function getPortSnapshot(portCodeOrName: string): PortOperationalSnapshot {
...
export function getNationalStressScore(): number {
  const meanCongestion =
    portRisks.reduce((sum, risk) => sum + risk.congestion, 0) / Math.max(1, portRisks.length);
  const severeLoad = portRisks.filter((risk) => risk.risk === "severe").length / Math.max(1, portRisks.length);
